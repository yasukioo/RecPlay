...
const sessionStoreState = vi.hoisted(() => ({
  state: "Unknown" as "Unknown" | "RecordingPaused" | "PlaybackPaused",
  duration_ns: 0,
  position_ns: 0,
  speed: 1,
}));
...
describe("TransportBar", () => {
  it("renders localized playback-paused labels for Simplified Chinese backend states", () => {
    sessionStoreState.state = "PlaybackPaused";
    ...
    expect(html).toContain("回放已暂停");
    expect(html).not.toContain("PlaybackPaused");
